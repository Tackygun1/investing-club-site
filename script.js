...
function initMarketBackgroundCandles() {
  ...
  // Create background layer + canvas (no HTML changes needed)
  ...
  function frame() {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      const yO = mapY(c.open, min, max);
      const yC = mapY(c.close, min, max);
      const yH = mapY(c.high, min, max);
      const yL = mapY(c.low, min, max);
